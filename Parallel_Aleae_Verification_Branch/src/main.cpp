...
int main (int argc, char *argv[]) {
    const std::string usage = std::string("usage: ") + argv[0] + " <file1: state> <file2: reactions> <trials> <time> <verbosity> <steps> [--seed <uint>] [--csv <path>]";
    ...
    for (int i = 7; i < argc; ++i) {
        ...
        if (arg == "--seed") {
            if (i + 1 >= argc) {
                std::cerr << "--seed requires a value." << std::endl;
                return 1;
            }
            seed = static_cast<unsigned int>(std::stoul(argv[++i]));
        } else if (arg == "--csv") {
            if (i + 1 >= argc) {
                std::cerr << "--csv requires a path." << std::endl;
                return 1;
            }
            csv_path = argv[++i];
        } else {
    ...
    for (unsigned i = 0; i < num_chems; i++) {
        ...
        std::cout << (total_triggered_threshs[i]/(double)num_trials)*100 << "%)\n";
    }
    ...
    if (!csv_path.empty()) {
        ...
        if (!csv.is_open()) {
            ...
            for (unsigned i = 0; i < num_chems; ++i) {
                double thresh_pct = (total_triggered_threshs[i] / (double) num_trials) * 100.0;
                csv << "cuda" << ","
                    << seed << ","
                    << in_filename << ","
                    << r_filename << ","
