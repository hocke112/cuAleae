...
aleae_stoch(const biocr_t           biocr, // reactions
            const stoch_param_t     param, // simulation parameters
                  ...
                  output_stats_t    &stats,// simulation statistics
            int  steps)
{
   ...
   bool   done = false;
   while (!done && (param.time_lt < 0 || stats.time < param.time_lt) && (steps < 0 || stats.event_ct <= steps)) {
    ...
    if(stats.event_ct == steps)
	{
		...
		cout<<"Maximum steps reached" << endl;
		break;
	}
      }
   }
